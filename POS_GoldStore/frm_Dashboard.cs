...
namespace POS_GoldStore
{
    public partial class frm_Dashboard : Form
    {
        ...
        private void TodayDetails()
        {
            ...
            lab_CashReceiveable.Text = SQL.ScalarQuery("Select ((select SUM(CM_Bal) from TvuFrmCashMain where CM_Mode ='RECEIVE')+(select SUM(BalAmt) as Receiveable from TvuFrmCreditSaleOrder)) as Payabel");
            lab_CashPayable.Text = SQL.ScalarQuery("Select ((select SUM(CM_Bal) from TvuFrmCashMain where CM_Mode ='PAY')+(select SUM(BalAmt) as Receiveable from TvuFrmCreditPurchaseOrder)) as Payable");
